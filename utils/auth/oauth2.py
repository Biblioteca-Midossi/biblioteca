...
async def create_access_token(user_data: dict, device_id: str):
    ...
    async with RedisDatabase() as redis:
        async with redis.pipeline() as pipe:
            ...
            await pipe.hset(
                f"{user_id}:device:{device_id}:access:{access_token}",
                mapping=token_data,
            )
            await pipe.expire(
                f"{user_id}:device:{device_id}:access:{access_token}",
...
async def verify_token(token: str, device_id: str = None):
    pattern = (
        f"*:access:{token}" if not device_id else f"*:device:{device_id}:access:{token}"
    )
    async with RedisDatabase() as redis:
        ...
        user_data["id_utente"], user_data["ruolo"] = (
            int(user_data["id_utente"]),
            int(user_data["ruolo"]),
        )
        return user_data
