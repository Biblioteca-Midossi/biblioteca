...
from api.models.auth_models import UserForm, TokenRequest
from api.services.user_service import check_user_exists, create_user
...
async def refresh_token(request: Request):
    ...
    if not ref_token or not device_id:
        raise HTTPException(
            status_code=401, detail="Refresh token o device ID mancante"
        )
    ...
    response = JSONResponse({"message": "Token aggiornato con successo"}, 200)
...
async def logout(request: Request):
    try:
        ...
        _ref_token = request.cookies.get("refresh_token")
        device_id = request.cookies.get("device_id")
        ...
        response = JSONResponse(
            {"status": "successful", "message": "Successfully logged out!"}, 200
        )

    except Exception as e:
        log.error(f"Error logging out: {e}")
        response = JSONResponse({"status": "error", "message": "Logout failed"}, 500)
    finally:
...
async def auth_check(request: Request):
    NO_LOGIN_MODE = get_env("NO_LOGIN_MODE")
    if NO_LOGIN_MODE and NO_LOGIN_MODE.lower() in ["true", "1"]:
