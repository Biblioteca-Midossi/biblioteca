...
async def get_books(
    ...
    with PSQLDatabase() as db:
        ...
        for book in raw_books:
            ...
            authors = [
                f"{author['nome']} {author['cognome']}"
                for author in db.fetchall_to_dict()
            ]
            books.append(
                {
                    "titolo": book["titolo"],
...
async def get_book(book_id: int = Path(...)):
    ...
    **Raises**:
    ...
    with PSQLDatabase() as db:
        ...
        authors = [
            f"{author['nome']} {author['cognome']}" for author in db.fetchall_to_dict()
        ]
    ...
    try:
        ...
        with PSQLDatabase() as db:
            ...
            if "nomeAutore" in data and "cognomeAutore" in data:
                cursor.execute(
                    "delete from libro_autori where id_libro = %s", (book_id,)
                )
                ...
                if len(author_names) != len(author_surnames):
                    raise HTTPException(
                        status_code=400,
                        detail="Mismatch between number of names and surnames",
                    )
            ...
            if file and file.filename:
                ...
                cursor.execute(
                    "update libri set thumbnail_path = %s where id_libro = %s",
                    (file_path[2:], book_id),
                )
        ...
        raise HTTPException(
            status_code=400, detail=f"database error during book update: {e}"
        )
    except Exception as e:
        log.error(f"Unexpected error during book update: {e}")
        raise HTTPException(
            status_code=500, detail=f"Unexpected error during book update: {e}"
        )
    ...
    **Raises**:
    ...
    try:
        with PSQLDatabase() as db:
            ...
            return JSONResponse(
                {"message": "Libro eliminato con successo"}, status_code=200
            )
    except psycopg.Error as e:
        log.error(f"database error during book deletion: {e}")
        raise HTTPException(
            status_code=500, detail=f"database error during book deletion: {e}"
        )
    except Exception as e:
        log.error(f"Unexpected error during book deletion: {e}")
        raise HTTPException(
            status_code=500, detail=f"Unexpected error during book deletion: {e}"
        )
