...
from api.models.auth_models import UserProfileUpdate
    ...
    try:
        ...
        if file and file.filename and file.size:
            if file.content_type not in [
                ...
                raise HTTPException(
                    status_code=400,
                    detail="Invalid file type. Only JPEG, JPG, PNG, WEBP and GIF files are allowed",
                )

            if file.size > 5_000_000:  # 5MB
                raise HTTPException(
                    status_code=400, detail="File size should not exceed 5MB"
                )
            ...
            with PSQLDatabase() as db:
                ...
                cursor.execute(
                    "select profile_picture from utenti where id_utente = %s",
                    (user["id_utente"],),
                )
...
async def get_users(
    ...
    return JSONResponse({"users": users, "total": total_users}, 200)


@router.put("/update-user/{user_id}", dependencies=[Depends(verify_role(3))])
async def update_user(
    request: Request, auth_data: Annotated[dict, Depends(get_current_user)]
):
    user = auth_data["user"]
    new_tokens = auth_data["new_tokens"]
